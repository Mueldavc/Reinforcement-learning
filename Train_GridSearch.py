...
if not os.path.isdir(r'{}models'.format(PATH)):
    os.makedirs(r'{}models'.format(PATH))
# Create results folder
if not os.path.isdir(r'{}results'.format(PATH)):
    os.makedirs(r'{}results'.format(PATH))
...
def save_model_and_weights(agent, model_name, episode, max_reward, average_reward, min_reward):
    ...
    agent.model.save(r'{}models/{}'.format(PATH, checkpoint_name))
...
for i, m in enumerate(models_arch):
    ...
    agent = DQNAgent(r"M{}".format(i), env, m["conv_list"], m["dense_list"], m["util_list"])
    ...
    # Save dataFrames
    res.to_csv(r"{}results/Results.csv".format(PATH))
    avg_df.to_csv(r"{}results/{}-Results-Avg.csv".format(PATH, MODEL_NAME))
    max_df.to_csv(r"{}results/{}-Results-Max.csv".format(PATH, MODEL_NAME))
